...
## SNP centric SNP-max method (see Fu et al. for more details on the method)

An example to run the SNP-max procedure on data from five populations:
...
options_max <- list()
for(j in 1:npop){
  options_max[[j]] <- list(maf = .02, HWE.p = 0, min.marg.p=1e-20, gene.R2=0.9, chr.R2=0.9, huge.gene.R2=0.8, huge.chr.R2=0.8,
...
res_temp <- sARTP.multiPop.SNP(summary.files.list=study.list, pathway=pathway_file, family=family, 
                              ...
                              lambda.list=lambda.list, ncases.list=ncases.list, ncontrols.list = nctrls.list, options.list = options_max, options.merged=options.merged1)
```
